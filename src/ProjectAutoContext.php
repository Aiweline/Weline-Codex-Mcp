...
final class ProjectAutoContext
{
    ...
    public function describe(array $hookResult): array
    {
        ...
        $context = implode("\n", [
            ...
            $refreshNote,
            $readinessNote,
            'Architecture-first read contract: use get_edit_bundle as the only normal repository read entry. If the owning subsystem or call chain is unclear, begin with a discovery batch using the full task plus relevant symbols/module/kinds, with paths omitted only to discover unknown related files; then materialize all candidate paths and symbols in one broad batch.',
            'Context completion contract: reason over accumulated bundles before editing. If context is insufficient, collect every missing path, symbol, and semantic search goal, then request another broad get_edit_bundle batch; never fall back to single-file or native per-file reads.',
