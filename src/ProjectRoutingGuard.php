            ...
            return [
                'hookSpecificOutput' => [
                    ...
                    'additionalContext' => sprintf(
                        ...
                        . "Follow an architecture-first batch workflow: when the owning subsystem or call chain is unclear, start with a discovery bundle using the full task plus relevant symbols/module/kinds, and omit paths only when intentionally searching the global index for unknown related files. Then materialize all known candidate paths and affected symbols in one broad bundle. After each bundle, reason over the accumulated context before editing. If context is incomplete, first collect every missing path, symbol, and semantic search goal, then issue another broad get_edit_bundle batch. Never read one file at a time or replace the indexed bundle with native per-file reads. Once context is sufficient, submit one complete edit-plan.v1 through apply_compact_edit.\n"
