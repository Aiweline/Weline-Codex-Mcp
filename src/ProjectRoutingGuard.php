            ...
            return [
                'hookSpecificOutput' => [
                    ...
                    'additionalContext' => sprintf(
                        ...
                        . "The local index for %s is current at revision %d. Use mcp__weline_project_intelligence__get_edit_bundle as the only normal read path before native repository discovery or code/document reads.\n"
                        . "Follow an architecture-first batch workflow: send the full task plus every known path and symbol in one bundle. When the owning subsystem or call chain is unclear, omit paths so that this same server call discovers and materializes all related candidates internally. The MCP receives the complete TaskContract and aggregates architecture, definitions, callers, callees, contracts, configuration, tests, docs, consumers, candidate paths, and semantic goals inside the same get_edit_bundle call. When ready_for_edit=true, submit one complete edit-plan.v1 through apply_compact_edit exactly once; do not call get_edit_bundle again, read repository files individually, or ask for intermediate user confirmation. On INDEX_NOT_READY or CONTEXT_INCOMPLETE, preserve the structured status and do not start model-driven continuation, native fallback, or user involvement. After success, review every changed file in one read-only pass from change_report.files[].diff and its hunk line numbers; report severity, path, new-line reference, rationale, and suggested fix, or explicitly state no findings plus residual validation gaps. Use get_edit_status only if the apply result is unavailable. Do not delegate editing. Only typed CONFLICT_REPLAN, IMPACT_EXPANSION, VALIDATION_REPAIR, or USER_SCOPE_CHANGE may recurse within the returned budgets; preserve unchanged operations, stop the same error at three, and keep each retry as one complete edit-plan.v1 transaction.\n"
                        . "Direct reads are exceptions only for higher-priority mandatory AGENTS/Skill material, an explicitly supplied user file, excluded/binary/unindexed material, validation, or an MCP result that reports stale/unavailable/insufficient context and cannot satisfy the next indexed batch. For a Shell exception, declare WELINE_MCP_DIRECT_READ_REASON=mandatory|user-file|unindexed|validation|mcp-fallback in the command so the bypass is auditable.",
                        $project['repository'],
                        $project['revision'],
                    ),
                ],
