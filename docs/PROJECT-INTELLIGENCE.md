...
`get_edit_bundle` 返回 `request_id/query_id`、project/index revision、freshness、任务摘要/摘要 Hash、精确 `regions[]`、文件/内容 Hash、行号、符号影响摘要和最多 3 个匹配 Skill 位置。显式 `symbols[]` 同时进入检索查询和批量影响分析；请求 class/interface/trait/enum 时通过 `symbols_parent_idx` 同批补充最多 4 个按任务相关度排序的成员实现，避免只拿到类声明壳。已知 `paths[]` 时先完成 `explicit_paths_refresh`，再一次物化 scoped chunks 并跳过全局 FTS/trigram/sparse。`on_demand_index` 返回本次核对路径、实际变化、耗时和刷新后 revision；`routing.batch_request` 回显批次形态。`performance.retrieval.timing_ms` 直接透传 total、exact、FTS、trigram/CJK、sparse、反馈和物化阶段，`performance.retrieval.sparse` 记录特征剪枝前后数量、估算 posting 数和两段耗时，外层 `performance.phases_ms` 继续记录 bundle 的检索、压缩、影响和 Skill 阶段。

推荐调用协议是“架构发现 + 批量物化 + 必要时少量宽批次补齐 + 一次本地写”。所属子系统或调用链未知时，AI 先以完整任务、相关符号/module/kinds 发起不带路径围栏的发现批次；候选明确后，把全部已知路径和符号一次传给 `get_edit_bundle` 物化。每轮 bundle 后，模型必须先在云端判断累计上下文是否足以列出完整修改文件；不足时先汇总所有缺失路径、符号和语义搜索目标，再发起下一批，禁止逐文件读取。上下文完整后只返回一份覆盖全部文件的 Replacement 计划，并调用一次 `apply_compact_edit`。后者保留 Seal/Token/HEAD/Hash 边界，先 Apply 和固定验证，再对最终状态只索引一次；失败默认回滚。
...
- `get_edit_bundle`：唯一常规读取入口；支持“无路径的架构/关联发现批次”和“携带全部已知路径/符号的物化批次”。每轮返回精确区域、Hash、影响、文档、Skill 位置和结构化下一步；symbol region 额外返回可直接写入 `edit-plan.v1` 的 `expected_file_sha256`、`symbol_uid`/`target_ref`、真实 body `expected_digest`，而 `content_sha256` 只表示片段。`structuredContent` 是权威结构，同时在文本 `content` 中镜像同一份有界 bundle，兼容只转发内容块的 Codex deferred-tool 包装。
