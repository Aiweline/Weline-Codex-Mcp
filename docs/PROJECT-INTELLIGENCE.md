...
`get_edit_bundle` 先返回 `architecture`、`candidate_paths`、`candidate_roles`、`coverage` 与 `continuation`，再返回精确 `regions[]`、文件/内容 Hash、行号、符号影响摘要和匹配 Skill。候选池在发现阶段默认扩大到 48–96 个结果，并按入口、视图模板、Provider/Query、Service、契约、配置、模型、工具协议、检索器、插件清单和文档等职责保证首批多样性；精确标识符与已知产品/模块别名获得额外权重，任务中的通用“修改”不再扩展为容易误命中业务 `edit()` 的词。显式 `symbols[]` 同时进入检索查询和批量影响分析，最多处理 24 个目标；批量影响不再只给计数，而会把三层词法上游图的具体相对路径并入候选清单。为防止 `Scope/Query/Config/Index` 等非显式通用符号把整个仓库拖入续批，只有用户显式目标或非通用命中的上游路径会进入候选清单。请求 class/interface/trait/enum 时仍通过 `symbols_parent_idx` 同批补充成员实现。已知 `paths[]` 时先完成 `explicit_paths_refresh`，物化结果按请求路径 round-robin：先让每个有候选的路径占一个区域，再继续分配剩余区域，避免高分文件独占 `max_regions`；随后跳过全局混合检索。`continuation.next_path_batches` 每批最多 50 个已知未物化路径，`next_search_goals` 把所有缺失职责合并为一次无路径发现请求。`on_demand_index`、`routing.batch_request` 与 `performance` 继续回显刷新、批次和各检索阶段。

推荐调用协议是“架构发现 + 批量物化 + 必要时少量宽批次补齐 + 一次本地写”。所属子系统或调用链未知时，AI 先以完整任务、相关符号/module/kinds 发起不带路径围栏的发现批次；MCP 用职责推断扩展检索目标，把语义候选、精确符号和具体上游代码图路径合并后，先告诉 AI 已覆盖与仍缺失的架构职责。AI 必须先消费 `candidate_paths/candidate_roles/coverage/continuation`，再判断 `regions[]` 是否足够；`next_path_batches` 中的路径整批物化，所有 `next_search_goals` 合并为一次发现调用，禁止逐文件、逐职责询问。上下文完整后只返回一份覆盖全部文件的 Replacement 计划，并调用一次 `apply_compact_edit`。后者保留 Seal/Token/HEAD/Hash 边界，先 Apply 和固定验证，再对最终状态只索引一次；失败默认回滚。
...
- `get_edit_bundle`：唯一常规读取入口；支持“无路径的架构/关联发现批次”和“携带全部已知路径/符号的物化批次”。响应先给职责化 `architecture/candidate_paths/candidate_roles/coverage/continuation`，候选来源包含语义检索、精确符号、显式路径和具体上游代码图路径；随后给精确区域、Hash、影响、文档与 Skill。`next_path_batches` 必须整批请求，所有 `next_search_goals` 必须合并成一次发现，不能逐文件或逐职责读取。symbol region 继续返回 `expected_file_sha256`、`symbol_uid`/`target_ref` 与真实 body `expected_digest`；`structuredContent` 是权威结构，同时在文本 `content` 中镜像同一份有界 bundle。
- `apply_compact_edit`：唯一常规写入口；本地 Seal、Apply、Validate、Reindex，验证失败默认回滚；错误响应也完整镜像到文本 `content`，不会丢失可执行重规划详情。
- `get_edit_status`：恢复/诊断既有事务。
- `rollback_edit`：显式恢复仍满足 postimage guard 的事务。
- `health`：运行状态与能力诊断。

两个响应通道来自同一次本地索引查询或同一次错误封装，不是第二次读取，也不应在 UI 中被解释为逐文件读取。成功的 `get_edit_bundle` 与所有错误会在两个通道完整镜像；其他成功响应的文本通道仍保持短摘要。调用方只需向模型转发其中一份完整 payload；不得只保留短回执后再调用原生 Read 补偿。
