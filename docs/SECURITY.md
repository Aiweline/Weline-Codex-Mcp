...
2. Prepare 解析本地目标，检查 allowed roots/denied paths/symlink/文件与总字节预算、唯一 Anchor、expected digest、HEAD 和 Index revision。默认编辑根包含业务模块、MCP 自身和 `dev/ai/codex/tasks` 任务记录；任务目录的 `artifacts/**` 继续显式拒绝，其他密钥、生成物、第三方和安全路径门禁不变。
...
5. Apply 获取按固定顺序的文件级与项目级 `flock`，复核 read-set，把 preimage 写到私有 Journal，再同目录 temp + rename；`.lock` 文件内容仅是诊断元数据，所有权只由内核锁决定。
6. 锁冲突只做有界非阻塞轮询；超时返回 `EDIT_LOCK_TIMEOUT`，不会根据 PID、mtime 或锁文件存在性强制解锁。
7. Token 在 Apply 后失效；紧凑链路先针对 postimage 运行固定验证，不先索引尚未验证的中间状态。
8. 下次编辑或状态查询会在相同锁顺序下按 pre/postimage Hash 恢复中断事务；未知 Hash 转 `recovery_required`，绝不覆盖。
9. 成功时只定点索引 postimage 一次；验证失败或中断回滚时先依 Hash guard 恢复，再索引 preimage 一次。Rollback 仍不得覆盖用户后续修改。
