...
AI 不应对候选文件逐个发起读取，也不应先取路径再读取整文件。所属架构未知时先做不带路径围栏的发现批次；候选明确后，把任务、所有已知路径与符号一次交给 `get_edit_bundle` 物化：
...
响应只包含命中的 `regions[]`、文件/内容 Hash、行号、符号影响、Skill 位置和结构化 routing；默认最多 8 个区域、每文件最多 2 个区域。AI 必须先判断累计上下文：若不足，先汇总所有缺失路径、符号和语义搜索目标，再发起下一次宽批次或提高有界预算；不能隐式循环 N 次，更不能读取单个整文件。
...
- `get_edit_status` 会先尝试恢复指定事务，再返回事务、recovery 和 validation runs；`rollback_edit` 仍受 postimage guard 保护，`ROLLBACK_STALE` 或未知 Hash 必须人工检查。
...
- 已经拿到多个精确路径时，把完整 `paths[]/symbols[]` 一起放进同一个物化批次；累计上下文后来暴露更多目标时，再把新增目标合并成下一批，始终不要改回逐文件读取。
