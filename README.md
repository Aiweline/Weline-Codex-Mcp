...
主流程收敛为：**架构发现批次 → 相关路径/符号物化批次 → 必要时少量成批补齐 → 一次 `apply_compact_edit` 批量提交。**
...
能减少重复扫描、工具往返和上下文 Token。首次由 MCP 本地读取符合规则的 Git 文件并建立持久索引；后续主路径固定为“少量宽批次读、一次事务写”：

1. 所属架构或调用链不明确时，AI 先用完整任务、相关符号/module/kinds 发起架构发现批次；只有在搜索未知关联路径时才暂不传 `paths[]`，让 MCP 使用全局混合检索。
2. 候选路径明确后，AI 把全部相关 `paths[]` 和符号一次提交给 `get_edit_bundle`；MCP 定点刷新并批量返回精确代码/文档区域、Hash、影响摘要和 Skill 位置，不返回整文件。
3. AI 先在云端合并并判断累计上下文是否足够；不足时先汇总一批缺失路径、符号和语义搜索目标，再发起下一次宽批次，禁止退化为逐文件读取。
4. 累计上下文足够后，Codex 一次生成覆盖全部修改文件的 `edit-plan.v1` Replacement，并只调用一次 `apply_compact_edit`；PHP 按 project-relative path 排序获取跨进程文件锁，在锁内完成 Seal、原子写入、固定验证、定点重索引和失败自动回滚，结束路径立即释放全部锁。

每个 bundle 会在 `regions[]` 之前返回 `architecture`、`candidate_paths`、`candidate_roles`、`coverage` 和 `continuation`。候选清单合并语义召回、精确符号、显式路径以及代码图中的具体上游路径，并按入口、视图、Provider/Query、Service、契约、配置、模型、工具协议、检索器、插件清单和文档等职责去重。若当前片段未覆盖全部候选或推断职责，`continuation.next_path_batches` 直接给出最多 50 路径一批的物化请求，`next_search_goals` 则要求把所有缺失职责合并成一次发现请求；AI 不再从单个片段猜下一个文件。
...
```markdown
- Use `get_edit_bundle` for an architecture-discovery batch when targets are unknown, then materialize all known paths/symbols in broad batches; make another batch only when accumulated context is insufficient.
