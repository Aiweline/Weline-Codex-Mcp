...
macOS / Linux は `install.sh`、Windows は `start.bat` を GitHub または Gitee から一度ダウンロードして実行します。管理ディレクトリから `uninstall` を実行すると登録を削除し、`--purge-data` を指定しない限りインデックスと学習データを保持します。
